...
def run_swift_ane(redis_key, model_name):
    ...
    swift_ane_dir = "../swift/swift_compiled"
    command = ["./swift_yolo", redis_key, f"{redis_key}_result", "ww_pool"]
...
def run_command(redis_key, video, model_name, region_points, end_quit):
    command = ["python", "main_sahi.py", "--camera-name", redis_key, "--source", video, "--view-img", "--weights", model_name, "--region", region_points]
...
def main():
    ...
    main_process = run_command(args.camera_name, args.video, args.model_name, args.region, args.end_quit)
