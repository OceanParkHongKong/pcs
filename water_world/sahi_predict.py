...
from swift.pipe_utils import DetectionPipeReader, FramePipeWriter
...
def get_prediction(
    ...
    verbose: int = 0,
    pipe_reader: DetectionPipeReader = None,
    pipe_writer: FramePipeWriter = None,
) -> PredictionResult:
    """
    ...
    # image_as_numpy = np.array(image_as_pil)
    # Convert RGB to BGR (OpenCV uses BGR color order)
    # image_as_numpy = cv2.cvtColor(image_as_numpy, cv2.COLOR_RGB2BGR)
    # cv2.imshow("image_as_pil", image_as_numpy)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    
    # Convert PIL Image to NumPy array if needed
    image_np = np.array(image_as_pil)

    # If the image is in RGB format and you need BGR for OpenCV
    if image_np.shape[2] == 3:  # Check if it has 3 channels
        # No need to convert to BGR since your Swift code expects RGB
        pass

    object_prediction_list = None
    # Now send the NumPy array to the pipe
    if pipe_writer is not None: #why at the of slice loop, pipe_writer is None?
        # cv2.imshow("image_np", image_np)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        
        pipe_writer.put_frame_to_pipe(image_np)
        detection_string = pipe_reader.read_from_pipe()
        # print("detection_string length ", len(detection_string))
        # print("detection_string: ", detection_string)
        
        # Use the new function to convert detection string to object predictions
        object_prediction_list = convert_detection_string_to_object_predictions(
            detection_string, 
        ...
        # print(f"Parsed {len(object_prediction_list)} object predictions from pipe")
    # else:
        # If pipe_writer is None, use the model's predictions
...
def get_sliced_prediction(
    ...
    if num_slices > 1 and perform_standard_pred:
        ...
        if prediction_result is not None and prediction_result.object_prediction_list is not None:
...
def convert_detection_string_to_object_predictions(detection_string, full_shape=None, shift_amount=None, slice_height=1024, slice_width=1024):
    """
    Convert a detection string from the pipe into a list of ObjectPrediction objects.
    
    Args:
        detection_string (str): The detection string from the pipe
        full_shape (list, optional): The full shape of the image [height, width]
        shift_amount (list, optional): The shift amount [shift_x, shift_y]
        
    Returns:
        List[ObjectPrediction]: A list of ObjectPrediction objects
    """
    from sahi.prediction import ObjectPrediction
    
    object_prediction_list = []
    
    # Create a simple category name to ID mapping
    category_map = {
        "head": 0,
        "person": 1,
        "vehicle": 2,
    ...
    # Use default shift_amount if not provided
    if shift_amount is None:
        shift_amount = [0, 0]
    
    # Split the string by } { to get individual detection entries
    detection_entries = detection_string.replace("} {", "}|{").split("|")
    # print(f"Found {len(detection_entries)} detection entries")
    
    for i, entry in enumerate(detection_entries):
        # Clean up the entry and convert to dictionary
        try:
            ...
            # Handle missing braces if needed
            if not entry.startswith("{"):
                entry = "{" + entry
            if not entry.endswith("}"):
                entry = entry + "}"
            
            # Extract the bbox, confidence, and label directly using string operations
            # This avoids issues with splitting by commas inside the bbox
            bbox_match = entry.find('"bbox":"(') 
            if bbox_match != -1:
                bbox_start = entry.find('(', bbox_match) + 1
                bbox_end = entry.find(')', bbox_start)
                bbox_str = entry[bbox_start:bbox_end]
                
                # Extract confidence
                conf_match = entry.find('"confidence":"')
                if conf_match != -1:
                    conf_start = conf_match + 14  # length of '"confidence":"'
                    conf_end = entry.find('"', conf_start)
                    confidence_str = entry[conf_start:conf_end]
                    
                    # Extract label
                    label_match = entry.find('"label":"')
                    if label_match != -1:
                        label_start = label_match + 9  # length of '"label":"'
                        label_end = entry.find('"', label_start)
                        label = entry[label_start:label_end]
                        
                        # print(f"Extracted - bbox: {bbox_str}, confidence: {confidence_str}, label: {label}")
                        
                        try:
                            # Parse bbox coordinates
                            bbox_parts = [part.strip() for part in bbox_str.split(',')]
                            if len(bbox_parts) >= 4:
                                x = float(bbox_parts[0])
                                y = float(bbox_parts[1])
                                w = float(bbox_parts[2])
                                h = float(bbox_parts[3])
                                
                                # Convert normalized coordinates to absolute coordinates
                                img_height, img_width = slice_height, slice_width
                                # print ("full_shape, shift_amount", full_shape, shift_amount)
                                
                                w_abs = w * img_width
                                h_abs = h * img_height                           
                                x_abs = x * img_width
                                y_abs = (1- y) * img_height - h_abs

                                
                                # Create bbox as [xmin, ymin, xmax, ymax]
                                bbox = [x_abs, y_abs, x_abs + w_abs, y_abs + h_abs]
                                
                                # Get category_id from map or use default 0
                                ...
                                # print(f"Successfully added object prediction {i}")
                            else:
                                print(f"Not enough bbox parts: {bbox_parts}")
                        except Exception as e:
                            print(f"Error processing bbox values: {e}")
            else:
                print(f"Could not find bbox in entry {i}")
        except Exception as e:
            print(f"Error processing entry {i}: {e}")
            continue
    
    # print(f"Final object_prediction_list length: {len(object_prediction_list)}")
