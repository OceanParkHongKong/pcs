...
class Annotator:
    ...
    def count_labels(self, in_count=0, out_count=0, count_txt_size=2, color=(255, 255, 255), txt_color=(0, 0, 0)):
        ...
        # text_x1 = (self.im.shape[1] - text_width - 120 * self.tf) // 2 - gap - 100 #middle 
        # text_x2 = (self.im.shape[1] - text_width + 120 * self.tf) // 2 + gap - 100 #middle 
        text_x1 = (self.im.shape[1] - text_width - 120)- gap - 100 #right
        text_x2 = (self.im.shape[1] - text_width + 120)- gap - 100 #right 
        # text_y = self.im.shape[0] - t_size_out[1] #bottom
        text_y = 100 #top

        text_y_in = text_y - t_size_in[1] * 2
        
        # Create a rounded rectangle for in_count
        cv2.rectangle(self.im, (text_x2 - 5, text_y_in - 5), (text_x2 + text_width + 7, text_y_in + t_size_in[1] + 7), color,
                      -1)
        cv2.putText(self.im,
                    str(in_count), (text_x2, text_y_in + t_size_in[1]),
                    0,
                    tl / 2,
                    txt_color,
                    self.tf,
