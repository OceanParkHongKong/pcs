#Copied from boxmot.trackers.bytetrack.BYTETracker
...
class BYTETracker(BaseTracker):
    ...
    def update(self, dets, im=None, embs=None):
        # if len(dets) > 0:
        #    print (f"larry in tracker 1: {(dets[0])}")
        assert isinstance(
            dets, np.ndarray
        ...
        # print (f"larry {len(dets)}")
        
        # if len(dets) > 0:
        #    print (f"larry in tracker {(dets[0])}")
        ...
        #https://github.com/ifzhang/ByteTrack/issues/328
        if len(self.removed_stracks) > 1000:
        #    print ("++++++++++ remove tracked more than 1000 ++++++++++")
